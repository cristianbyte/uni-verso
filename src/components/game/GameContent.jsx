...
const GameContent = ({ songData }) => {
  ...
  const [hasRefreshedAudioPreview, setHasRefreshedAudioPreview] =
    useState(false);
  const [hasHandledAudioError, setHasHandledAudioError] = useState(false);
  const [youtubeVideoId, setYoutubeVideoId] = useState(null);
  const [youtubeStatusMessage, setYoutubeStatusMessage] =
    useState("Loading video...");
  const [progressPercentage, setProgressPercentage] = useState(0);
  const totalVerses = songData.song.verseCount;

  const refreshAudioPreview = useCallback(
    async ({ showUnavailableAlert = false } = {}) => {
      try {
        const searchResult = await fetchSuggestions(
          `${songData.song.artist} ${songData.song.title}`,
        );
        const matchingSong =
          searchResult.data?.find((song) => song.id === songData.song.id) ||
          searchResult.data?.[0];
  ...
  const handleEnding = async () => {
    try {
      ...
      const response = await submitLines(
        user,
        finalVerseList,
        songData.pairingCode,
      );
      if (response) {
        ...
        <div className="game game--with-video">
          ...
          <div className="game__info">
            <div className="info__frame">
              <Frame
                src={pairedPlayer.image}
                text={pairedPlayer.name}
                fontSize={"1rem"}
              />
            </div>
          ...
          <div className="game__video-area">
            <YoutubePlayer
              ...
              statusMessage={youtubeStatusMessage}
            />
          </div>
